...
class CustomBaseUser(AbstractUser):
    ...
    user_type = models.CharField(
        ...
        default="player",
        help_text="Type of user: admin or player"
    )
    groups = models.ManyToManyField(
        Group,
        related_name="custombaseuser_set",  # <-- unique related_name
        blank=True,
    ...
    def save(self, *args, **kwargs):
        if not self.team_code:
            ...
            count = Team.all_objects.filter(
                ...
                sport=self.sport,
                sport__gender=self.sport.gender
            ).count() + 1
            self.team_code = f"{college_code}-{sport_code}-{gender_code}-{count}"
        self.full_clean()
